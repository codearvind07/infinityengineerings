              ...
              <div>
                ...
                >
                  <div className="bg-sphere-navy-dark/50 backdrop-blur-sm border border-sphere-blue-light/20 rounded-full px-4 py-1.5">
                    <span className="text-sphere-white text-sm font-medium">SECURITY SYSTEMS</span>
                  </div>
                </motion.div>
                
                <h1 className="text-4xl md:text-6xl font-bold text-sphere-white mb-6">
                  Door Hardware & 
                  <span className="block text-sphere-blue-light">Security Solutions</span>
                </h1>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {hardwareTypes.map((type, index) => (
              <motion.div
                ...
                className="bg-sphere-navy-dark/50 backdrop-blur-sm border border-sphere-blue-light/20 rounded-xl p-8 transition-all duration-300"
                whileHover={{ y: -5 }}
              >
                ...
                <div>
                  <h4 className="text-lg font-semibold text-sphere-blue-light mb-3">Applications:</h4>
      ...
      <section className="py-16 relative">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
            {specifications.map((spec, index) => (
              <motion.div
                ...
                viewport={{ once: true }}
                transition={{ duration: 0.5, delay: index * 0.1 }}
                className="bg-sphere-navy-dark/50 backdrop-blur-sm border border-sphere-blue-light/20 rounded-xl p-6 text-center transition-all duration-300"
                whileHover={{ y: -5 }}
              >
                <div className="bg-sphere-navy-dark/50 backdrop-blur-sm border border-sphere-blue-light/30 rounded-lg p-3 w-fit mx-auto mb-4">
                  <spec.icon className="h-8 w-8 text-sphere-blue-light" />
                </div>
                <h3 className="text-lg font-semibold text-sphere-white mb-2">
                  {spec.label}
                </h3>
                <p className="text-sphere-white/80">{spec.value}</p>
              </motion.div>
            ))}
          </div>
          ...
          <motion.div
            ...
            className="bg-sphere-navy-dark/50 backdrop-blur-sm border border-sphere-blue-light/20 rounded-xl p-8 transition-all duration-300"
          >
            <h3 className="text-2xl font-bold text-sphere-white mb-6 text-center">
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {certifications.map((cert, index) => (
                <motion.div
                  ...
                  className="flex items-center space-x-3 bg-sphere-navy-dark/50 backdrop-blur-sm border border-sphere-blue-light/30 rounded-lg p-4"
                >
                  <Star className="h-5 w-5 text-sphere-blue-light flex-shrink-0" />
                  <span className="text-sphere-white/80">{cert}</span>
                </motion.div>
              ))}
            </div>
          </motion.div>
      ...
      <section className="py-16 relative">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.div
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.8 }}
            className="bg-sphere-navy-dark/50 backdrop-blur-sm border border-sphere-blue-light/20 rounded-2xl p-12 transition-all duration-300"
          >
            <h2 className="text-3xl md:text-4xl font-bold text-sphere-white mb-6">
              Secure Your Facility Today
            </h2>
            <p className="text-xl text-sphere-white/80 mb-8 max-w-2xl mx-auto">
